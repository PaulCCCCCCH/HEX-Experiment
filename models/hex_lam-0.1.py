...
class MyModel(object):
	...
	bag_of_word_in = tf.reduce_mean(tf.concat([bow_pre, bow_hyp], 1), 1)
        ...
        W_fc2 = tf.Variable(tf.random_normal([self.dim, self.bow_layer_size], stddev=0.1))
	...
	# Compute loss using yconv_contact_loss: [h_fc1, h_fc2] and yconv_contact_H: [0(pad2), h_fc2]
	...
	self.total_cost = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.y, logits=y_conv_loss)) + self.lam * tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.y, logits=y_conv_H))
	############### MY CODE ENDS #####
