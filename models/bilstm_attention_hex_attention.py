...
class MyModel(object):
	...
	# Compute loss using [h_fc1, h_fc2] and [0(pad2), h_fc2]
        ...
        ######## Temp, prevent invertible matrix #################
	self.distort= tf.random_normal([3], stddev=1e-10)
        ...
        temp = tf.matmul(y_conv_H + self.distort, y_conv_H, transpose_a=True)
	...
	cost_logits = y_conv_loss
        ...
        self.total_cost = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.y, logits=y_conv_loss) + self.lam * reg) # Cost
