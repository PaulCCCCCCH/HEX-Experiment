...
class MyModel(object):
	...
	# Bag-of-word input (flattened sentence embeddings)
	input_dim = self.sequence_length * self.dim * 2
	bow_pre = premise_in
	bow_hyp = hypothesis_in
	bag_of_word_in = tf.concat([bow_pre, bow_hyp], 1)
	bag_of_word_in = tf.reshape(bag_of_word_in, (-1, input_dim))
        
        # Bag-of-word input layer params
        W_fc2 = tf.Variable(tf.random_normal([input_dim, 3], stddev=0.1))
        b_fc2 = tf.Variable(tf.zeros(3, 1))
        h_fc2 = tf.matmul(bag_of_word_in, W_fc2) + b_fc2

	# Compute both cost and prediction using yconv_contact_H
        y_conv_pred = tf.nn.sigmoid(h_fc2)
        self.logits = y_conv_pred # Prediction

	self.total_cost = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.y, logits=self.logits)) # Cost
