...
class MyModel(object):
	...
	## note: embedding_dim and hidden_dim are both 300, used interchangeably	
        ...
        # Set to True if you want to plug weights back to the model 
        self.plugWeights = True
    ...
    def plug(self, sess):
        ...
        if self.plugWeights:
            ...
            print("plugging weights back")
            self.W_cl_2 = self.W_cl_2.assign(weights_from_split)
            self.b_cl = self.b_cl.assign(bias_from_split)
            sess.run(self.W_cl_2)
            sess.run(self.b_cl)
