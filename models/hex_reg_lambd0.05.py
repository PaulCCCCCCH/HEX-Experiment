...
class MyModel(object):
	...
	self.W_cl_1 = tf.Variable(tf.random_normal([self.dim, 3], stddev=0.1))
        ...
        reg = lambd * tf.reduce_sum(tf.square(self.W_cl_2)) / (2 * 50)
	...
	cost_logits = y_conv_loss
        self.total_cost = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.y, logits=y_conv_loss)) + reg # Cost
